...
const StateViz = (props: Props) => {
  ...
  const statesList = _.map(_.uniqBy(PopulationData, 'State_Name'), 'State_Name');
  const statePopulation: StatePopulationDataType[] = statesList.map((state) => {
    const districtsInState = _.filter(PopulationData, d => d.State_Name === state);
  ...
  const percentPopulationVaccinated = statesData.map(state => {
    ...
    const popIndx = statePopulation.findIndex(st => st.State_Name === state.State)
    return {
      State: state.State,
  ...
  const percentPopulationVaccinatedSorted = _.sortBy(percentPopulationVaccinated, d => d.Percent);
  ...
  return <>
    ...
    <div className="container">
      ...
      <SelectDiv>
        <Select
          options={options}
          onChange={(value: any) => { setSelectedState(value[0]) }}
    ...
    <StateDailyDose
      windowWidth={windowWidth}
      statesData={statesData[statesData.findIndex(state => state.State === selectedState.label)].Data}
      stateName={selectedState.label}
      statePopulation={statePopulation[statePopulation.findIndex(st => st.State_Name === selectedState.label)].State_Population}
    />
    <br />
    <ListViewForStates
