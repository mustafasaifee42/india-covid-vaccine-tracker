  ...
  useEffect(() => {
    if (GraphRef.current && GraphRef !== null && TooltipRef.current && TooltipRef !== null) {
      ...
      const yScale = scaleLinear()
        .range([0, graphHeight])
        .domain([max(data, (d: DataType) => d.Delta_Total_Dose_Administered), 0] as number[]);
      ...
      g.selectAll('.bars')
        ...
        .append('rect')
        .attr('class', 'bars')
        .attr('x', (d: DataType) => xScale(d.date) - barWidth / 2)
        .attr('height', (d: DataType) => graphHeight - yScale(d.Delta_Total_Dose_Administered))
        .attr('width', barWidth)
        .attr('y', (d: DataType) => yScale(d.Delta_Total_Dose_Administered))
        ...
        .on('mouseenter', (event, d) => {
          ...
          select(TooltipRef.current)
            .select('.totalDose')
            .html(new Intl.NumberFormat('en-US').format(d.Delta_Total_Dose_Administered))
          select(TooltipRef.current)
            .select('.avgDose')
            .html(new Intl.NumberFormat('en-US').format(parseFloat((d['7_Day_Ruuning_Avg_Total_Dose_Administered']).toFixed(2))))
      ...
      const lineGraph = line()
        ...
        .y((d: any) => yScale(d['7_Day_Ruuning_Avg_Total_Dose_Administered']))
        .curve(curveStep);
      g.append('path')
        .datum(data)
        .attr('class', 'line')
      ...
      g.append('circle')
        ...
        .attr('cy', yScale(data[data.length - 1]['7_Day_Ruuning_Avg_Total_Dose_Administered']))
        .attr('r', 2.5)
