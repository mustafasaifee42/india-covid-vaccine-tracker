    ...
    <>
      <div className="container">
        ...
        <div>
          ...
          <br />
          <br />
          At this rate the whole population of {districtName} will have one dose of vaccine in <span className="bold tags">{(((districtPopulation - districtData[districtData.length - 1].Cummilative_First_Dose_Administered) / districtData[districtData.length - 1]['7_Day_Ruuning_Avg_Total_Dose_Administered']) / 365).toFixed(1)} years</span> and fully vaccinated in next <span className="bold tags">{((((districtPopulation * 2 - districtData[districtData.length - 1].Cummilative_Total_Dose_Administered) / districtData[districtData.length - 1]['7_Day_Ruuning_Avg_Total_Dose_Administered']) / 365).toFixed(1))} years</span>. To fully vaccinate 70% (threshold estimated for herd immunity) of population <span className="bold tags">{((((0.7 * 2 * districtPopulation) - districtData[districtData.length - 1].Cummilative_Total_Dose_Administered) / districtData[districtData.length - 1]['7_Day_Ruuning_Avg_Total_Dose_Administered']) / 365).toFixed(1)} years</span> will be needed.
        </div>
      </div>
      <br />
